...
1. 支持“综合检索”和“高级检索”两种方式
2. 支持不同检索字段的组合
3. 支持按年份，引用量，相关性等多种排序方式
4. 支持筛选论文发表时间及设置不同检索字段优先级（按顺序）
...
检索模块实现的功能见[功能特色](#function)部分。

### 使用方法

展示端通过构造`search_info`，实现对不同字段的检索，并按指定优先级、排序方式返回检索结果，参考[代码调用](#code)。综合检索和高级检索的`query_type`分别是`integrated_search`和`advanced_search`。当检索到某篇包含视频的论文后，还可以进一步定位视频字幕信息。
